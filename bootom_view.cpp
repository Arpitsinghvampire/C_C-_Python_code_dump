//bottom_view.cpp
//here in place of the top view we find the bottom view that is find the last element 
//top_view.cpp
...
void assign_coordinates(node* root)
{
    ...
    for(auto &column : vertical_nodes) //print only the first element of each column
    {
        ...
        {
            cout<< *level.second.rbegin()<<endl; //rbegin returns the iterator , so we need to get the values 
        }
